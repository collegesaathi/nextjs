    ...
    return (<>
         <Layout>
            <div className="md:py-8">
                ...
                {/* Main Content Layout */}
                <div className="mx-auto container xl:max-w-[1430px] px-2 md:px-4">
                    <div className="flex flex-col lg:flex-row items-start pt-10 gap-6">
                        
                        {/* LEFT COLUMN: STICKY SIDEBAR */}
                        <aside className="w-full lg:w-3/12 lg:sticky lg:top-[100px] transition-all duration-300">
                            <div className="bg-[#f9fafb] rounded-xl lg:shadow-[4px_4px_4px_rgba(0,0,0,0.06)] overflow-hidden">
                                <FrontendSidebar data={data?.CourseData} />
                            </div>
                        </aside>

                        {/* RIGHT COLUMN: NATURAL SCROLLING */}
                        <main className="w-full lg:w-9/12 space-y-10">
                            {data?.CourseData?.about && <Aboutdetails about={data?.CourseData?.about} />}
                            {data?.CourseData?.fees && <UpdatedFee fees={data?.CourseData?.fees} />}
                            {data?.CourseData?.approvals && (
                            ...
                            {courseData?.data?.length > 0 && (
                                <CoursesSwiper 
                                    ...
                                    name={"specialisation"} 
                                    title={`${data?.CourseData?.name} - Specialisation`} 
                                />
                            )}
